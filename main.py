...
<head>
    ...
    <style>
        ...
        .score-box { background: #fff; border: 2px solid #f0f0f0; border-radius: 12px; padding: 15px; text-align: center; margin-bottom: 20px; transition: all 0.3s ease; }
...
<body>
    <div class="sidebar">
        <div class="header"><h2>HEALTH MONITOR</h2><p>Regional Risk Intelligence</p></div>
                ...
                L.geoJson(data, {
                    ...
                    onEachFeature: (f, l) => {
                        l.on('mouseover', function() {
                            const d = countryDatabase[f.properties.ISO_A3];
                            if(d) {
                                document.getElementById('location-tag').innerText = d.name.toUpperCase();
                                document.getElementById('score-text').innerText = d.score + "%";
                                document.getElementById('status-text').innerText = d.status;
                                document.getElementById('score-text').style.color = d.color;
